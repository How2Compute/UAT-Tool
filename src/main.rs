...
fn find_engine_version(installs: &Vec<EngineInstall>, name: &str) -> Option<EngineInstall> {
    if let Some(cl_version_str) = name.strip_prefix("cl-") {
        if let Some(cl_version_id) = cl_version_str.parse::<u32>().ok() {
            let res = installs
                .iter()
                .find(|item| item.cl_version == cl_version_id);
        ...
        mut_installs.sort_by_key(|item| {
            item.major_version as u32 * 10000
                + item.minor_version as u32 * 100
                + item.patch_version as u32
        });
        ...
        let major_version: Option<u8> = version_name
            .split('.')
            .nth(0)
            .or(Some(""))
            .unwrap()
            .parse()
            .ok();
        let minor_version: Option<u8> = version_name
            .split('.')
            .nth(1)
            .or(Some(""))
            .unwrap()
            .parse()
            .ok();
        let patch_version: Option<u8> = version_name
            .split('.')
            .nth(2)
            .or(Some(""))
            .unwrap()
        ...
        let res = installs
            .iter()
            .filter(|item| !needs_source_build || item.is_source)
            .find(|item| {
...
fn main() {
    ...
    let args = Args::parse();
    let engine_installs =
        engine_utils::get_engine_installs().expect("Unable to fetch engine installs: ");
    ...
    if args.should_list {
        ...
        for install in engine_installs {
            let version_string = format!(
                ...
                install.minor_version,
                install.patch_version,
                match install.is_source {
                    true => "[SOURCE]".bold(),
    ...
    // Regular run
        ...
        let engines_to_use: Vec<(&str, Option<EngineInstall>)> = requested_engine_names
            ...
            .map(|&version_name| {
                (
                    version_name,
                    find_engine_version(&engine_installs, version_name),
                )
            })
            .collect();
        let engines_not_found: Vec<&(&str, Option<EngineInstall>)> = engines_to_use
            .iter()
            .filter(|(_, engine)| engine.is_none())
            .collect();
        ...
        if !engines_not_found.is_empty() {
            ...
            println!(
                "{}:",
                "Unable to resolve the following engine version(s)"
                    .bold()
                    .red()
            );
        ...
        if engines_to_use.len() > 1 {
            println!(
                "{} {}",
                engines_to_use.len().to_string().bold().yellow(),
                "versions passed in!".bold().green()
            );
            println!(
                "{}",
                "We will run your command once for each version, in the order listed above"
                    .italic()
            );
        }
        ...
        for (name, engine) in engines_to_use {
            ...
            let exit_code = uat_invocation
                .wait()
                .expect("Unable to wait for UAT completion")
                .code()
                .expect("Unable to retrieve output code");
            println!(
                "{}: {}",
                "UAT process exited with code".bold(),
                exit_code.to_string().yellow().bold()
            );
            ...
            if exit_code != 0 {
                println!(
                    "{}",
                    "Non-zero exit code! Assuming process has failed, stopping further execution!"
                        .red()
                        .bold()
                );
                return;
